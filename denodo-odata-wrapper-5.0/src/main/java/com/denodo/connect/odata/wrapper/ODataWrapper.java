...
public class ODataWrapper extends AbstractCustomWrapper {
    ...
    private static final CustomWrapperInputParameter[] INPUT_PARAMETERS = new CustomWrapperInputParameter[] {
        ...
        new CustomWrapperInputParameter(INPUT_PARAMETER_VERSION, "Activate Compatibility (may not work)", 
                false, CustomWrapperInputParameterTypeFactory.enumStringType(new String[]{INPUT_PARAMETER_VERSION_1,INPUT_PARAMETER_VERSION_2})),
    ...
    public void run(CustomWrapperConditionHolder condition,
        ...
        try {
            ...
            if(logger.isDebugEnabled()){
                for(CustomWrapperFieldExpression field:projectedFields){
                    ...
                    if(!field.hasSubFields()){
                        CustomWrapperSchemaParameter[] schemaParameters = getSchemaParameters(inputValues);
                        int type = getSchemaParameterType(field.getStringRepresentation(), schemaParameters);
                        
                        
                        logger.info("Field/Value/Type: " + field.getStringRepresentation() + "/"
                                +  "/" + type);
                        logger.info("register/array/contains/condition/field/function/simple/subfield: " + field.isRegisterExpression() + "/"
            ...
            List<OEntity> response = getEntities(entityCollection, rels, consumer, condition, projectedFields, inputValues);
    ...
    @Override
    public int insert(Map<CustomWrapperFieldExpression, Object> insertValues, Map<String, String> inputValues)
        ...
        OCreateRequest<OEntity> request = consumer.createEntity(entityCollection);
        
        CustomWrapperSchemaParameter[] schemaParameters = getSchemaParameters(inputValues);

        for(CustomWrapperFieldExpression field:insertValues.keySet()){
    ...
    @Override
    public int update(Map<CustomWrapperFieldExpression, Object> updateValues,
            CustomWrapperConditionHolder conditions, Map<String, String> inputValues)
                    throws CustomWrapperException {
        try {
            int updated = 0;
            String entityCollection = (String) getInputParameterValue(INPUT_PARAMETER_ENTITY_COLLECTION).getValue();
            ...
            Map<CustomWrapperFieldExpression, Object>  conditionsMap = conditions.getConditionMap(true);
            List<CustomWrapperFieldExpression> conditionsList = new ArrayList<CustomWrapperFieldExpression>();
            conditionsList.addAll(conditionsMap.keySet());
            
            List<OEntity> response = getEntities(entityCollection, new ArrayList<String>(), null, conditions,
                    conditionsList, inputValues);
    
            ODataConsumer consumer = getConsumer();
            for(OEntity oEntity:response){
                OModifyRequest<OEntity> request = consumer.updateEntity(oEntity);

                CustomWrapperSchemaParameter[] schemaParameters = getSchemaParameters(inputValues);

                for(CustomWrapperFieldExpression field:updateValues.keySet()){
    ...
    @Override
    public int delete(CustomWrapperConditionHolder conditions, Map<String, String> inputValues)
            throws CustomWrapperException {
        ...
        try {
            ...
            if(conditionsMap != null){
                ...
                for(OEntity oEntity:response){
                    ...
                    OEntityDeleteRequest request = consumer.deleteEntity(oEntity);
                    request.execute();
                    deleted++;
                }
                return deleted;
    ...
    private List<OEntity> getEntities(String entityCollection, List<String> rels, ODataConsumer consumer,
            CustomWrapperConditionHolder condition, List<CustomWrapperFieldExpression> projectedFields, Map<String, String> inputValues){
        ...
        OQueryRequest<OEntity> request = consumerLocal.getEntities(entityCollection);
        ...
        if (conditionMap != null && !conditionMap.isEmpty()) {
            // Simple condition
            String simpleFilterQuery = ODataQueryUtils.buildSimpleCondition(conditionMap, rels);
            ...
            // Complex condition
            String complexFilterQuery = ODataQueryUtils.buildComplexCondition(condition.getComplexCondition(), rels);
        ...
        Enumerable<OEntity> requestUpdated = request.execute();
    ...
    private static Map<String, EdmEntitySet> getEntitySetMap(ODataConsumer consumer) {
    ...
    private static int getSchemaParameterType(String nameParam, CustomWrapperSchemaParameter[] schemaParameters){
        for(CustomWrapperSchemaParameter param: schemaParameters){
    ...
    private static String getSchemaParameterName(String nameParam, CustomWrapperSchemaParameter[] schemaParameters){
        for(CustomWrapperSchemaParameter param: schemaParameters){
            if(nameParam.equalsIgnoreCase(param.getName())){
    ...
    private static CustomWrapperSchemaParameter[]
            getSchemaParameterColumns(String nameParam, CustomWrapperSchemaParameter[] schemaParameters){
        for(CustomWrapperSchemaParameter param: schemaParameters){
            if(nameParam.equalsIgnoreCase(param.getName())){
