...
public class MetadataAccessor {
    ...
    public List<Property> getEntityProperties(final FullQualifiedName entityName) throws SQLException {
        ...
        try {
            ...
            while (columnsRs.next()) {
                ...
                final EdmSimpleTypeKind type = SQLMetadataUtils.sqlTypeToODataType(sqlType, vdpVersion);
                ...
                if (type == EdmSimpleTypeKind.String 
                        && sqlType != VDPJDBCTypes.INTERVAL_YEAR_MONTH
                        && sqlType != VDPJDBCTypes.INTERVAL_DAY_SECOND) { // because column size for INTERVAL VDP types 
                ...
                // Don't establish precision for DateTime, Time and DateTimeOffset types because VDP returns usually
                // 19 as max length and Olingo will display additional zeroes like
                // 2006-02-15T03:03:42.00000000000+01:00 (EdmDateTime.appendFractionalSeconds method)
    ...
    public Key getEntityPrimaryKey(final FullQualifiedName entityName) throws SQLException {
        ...
        try {
            ...
            entityPrimaryKeyProperties = buildKeys(columnsRs, false);
            ...
            if (entityPrimaryKeyProperties.isEmpty()) {
                columnsRs = metadata.getColumns(connection.getCatalog(), null, entityName.getName(), null);
                entityPrimaryKeyProperties = buildKeys(columnsRs, true);
            }
    ...
    private List<PropertyRef> buildKeys(final ResultSet columnsRs, final boolean autogeneratedPK) throws SQLException {
    ...
    public List<NavigationProperty> getEntityNavigationProperties(final FullQualifiedName entityName) {
    ...
    public List<Association> getAllAssociations(final String namespace) {
    ...
    public List<Association> getAssociationsForEntity(final FullQualifiedName entityName) {
        ...
        final List<Property> complexTypes =
                this.denodoTemplate.query(descComplexTypeQuery, new RowMapper<Property>() {
                    ...
                    public Property mapRow(final ResultSet rs, final int rowNum) throws SQLException {
                        ...
                        if (typeCode != 0) {  // When typeCode is zero it means that we do not have information about the type
                           
                            final int vdpVersion = rs.getStatement().getConnection().getMetaData().getDatabaseMajorVersion();
                            type = SQLMetadataUtils.sqlTypeToODataType(typeCode, vdpVersion);
                        }  
